...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    ...
    train_x = file_data['train_x'][0:32,:,:,:]  # (10181, 307, 3, 12)
    train_x = train_x[:, :, 0:1, :]
    train_target = file_data['train_target'][0:32,:,:]  # (10181, 307, 12)

    val_x = file_data['val_x'][0:32,:,:,:]
    val_x = val_x[:, :, 0:1, :]
    val_target = file_data['val_target'][0:32,:,:]

    test_x = file_data['test_x'][0:32,:,:,:]
    test_x = test_x[:, :, 0:1, :]
    test_target = file_data['test_target'][0:32,:,:]
